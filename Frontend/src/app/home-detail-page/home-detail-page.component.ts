...
export class HomeDetailPageComponent implements OnInit {
  ...
  constructor(private router: Router, private StorageService: StorageServiceService,public dialog: MatDialog,
  ...
  removeRoommate = () => {
    if(this.isowner){
      let thisDialogRef = this.dialog.open(RemoveRoommateDialogComponent, { data: { roommates: this.roommate_array, isOwner: this.isowner, houseId: this.home.houseId } });
    }else{
